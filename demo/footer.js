document.write('\
<div class="container-fluid footer p-r-60 p-l-60">\
  <div class="row p-t-20 p-b-20 ">\
    <div class="col-sm-3 p-t-10 p-b-10">\
      <h2 class="text-master no-margin"><span class="bold">Nahdlatul</span><br> Ulama</h2>\
    </div>\
    <div class="col-sm-3 p-t-10 p-b-10 pull-right ">\
      <h6 class="font-montserrat text-uppercase fs-14 text-master p-b-10">Company </h6>\
      <ul class="no-style">\
          <li class="m-b-5 no-padding"><a href="#" class="link text-master">Design</a></li>\
          <li class="m-b-5 no-padding"><a href="#" class="link text-master">Features</a></li>\
          <li class="m-b-5 no-padding"><a href="#" class="link text-master">Team</a></li>\
          <li class="m-b-5 no-padding"><a href="#" class="link text-master">Work Station</a></li>\
      </ul>\
    </div>\
    <div class="col-sm-3 p-t-10 p-b-10 pull-right">\
      <h6 class="font-montserrat text-uppercase fs-14 text-master p-b-10">Company </h6>\
      <ul class="no-style">\
          <li class="m-b-5 no-padding"><a href="#" class="link text-master">Design</a></li>\
          <li class="m-b-5 no-padding"><a href="#" class="link text-master">Features</a></li>\
          <li class="m-b-5 no-padding"><a href="#" class="link text-master">Team</a></li>\
          <li class="m-b-5 no-padding"><a href="#" class="link text-master">Work Station</a></li>\
      </ul>\
    </div>\
    <div class="col-sm-3 p-t-10 p-b-10 pull-right">\
      <h6 class="font-montserrat text-uppercase fs-14 text-master p-b-10">Company </h6>\
      <ul class="no-style">\
          <li class="m-b-5 no-padding"><a href="#" class="link text-master">Design</a></li>\
          <li class="m-b-5 no-padding"><a href="#" class="link text-master">Features</a></li>\
          <li class="m-b-5 no-padding"><a href="#" class="link text-master">Team</a></li>\
          <li class="m-b-5 no-padding"><a href="#" class="link text-master">Work Station</a></li>\
      </ul>\
    </div>\
  </div>\
  <div class="copyright sm-text-center">\
    <p class="small no-margin pull-left sm-pull-reset">\
      <span class="hint-text">Copyright &copy; 2016 </span>\
      <span class="font-montserrat"><a class="text-success" href="#">Nahdlatul Ulama</a></span>\
      <span class="hint-text">All rights reserved. </span>\
    </p>\
    <p class="small no-margin pull-right sm-pull-reset">\
      <a class="text-success" href="#">Dev-NU</a> <span class="hint-text">&amp; Made with Love ®</span>\
    </p>\
    <div class="clearfix"></div>\
  </div>\
</div>\
');
